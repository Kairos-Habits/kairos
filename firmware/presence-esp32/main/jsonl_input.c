...
// Called by calibration code to report progress
void jsonl_calibration_report_progress(int progress, int samples) {
    jsonl_emit_calibration_status("sampling", progress, samples);
}

// Called by calibration code when complete
void jsonl_calibration_report_complete(int recommended_threshold) {
    calibration_result = recommended_threshold;
    calibrating = false;
    jsonl_emit_calibration_status("complete", 100, recommended_threshold);
    ESP_LOGI(TAG, "Calibration complete, recommended threshold: %d", recommended_threshold);
}
